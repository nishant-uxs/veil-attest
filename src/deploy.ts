/**
 * Deploy VeilAttest to a Midnight network
...
// VeilAttest keeps the raw claim in private state via the privateClaim witness.
const PRIVATE_STATE_ID = 'veilAttestPrivateState';

const INITIAL_CLAIM = (() => {
  const bytes = new Uint8Array(32);
  bytes.set(Buffer.from('veil-attest-genesis-claim-v1'));
  return bytes;
})();
...
const zkConfigPath = path.resolve(__dirname, '..', 'contracts', 'managed', 'veil-attest');
...
const VeilAttest = await import(pathToFileURL(contractPath).href);

const compiledContract = CompiledContract.make('veil-attest', VeilAttest.Contract).pipe(
...
async function createProviders(walletCtx: WalletContext) {
  ...
  return {
    privateStateProvider: levelPrivateStateProvider({
      privateStateStoreName: 'veil-attest-state',
      accountId,
...
async function main() {
  console.log('\n╔══════════════════════════════════════════════════════════════╗');
  console.log(`║  Deploy VeilAttest to ${network}`);
  console.log('╚══════════════════════════════════════════════════════════════╝\n');
  ...
  // On public networks, waiting for all three sub-wallets (incl. DUST) can hang
  // forever when the indexer never marks dust progress complete. Prefer the
  // facade isSynced flag with a long timeout, then fall back to the latest state.
    ...
    state = await Promise.race([
      ...
      Rx.firstValueFrom(
        walletCtx.wallet.state().pipe(
          Rx.filter((s) => Boolean((s as any).isSynced)),
          Rx.timeout({ first: 180_000 }),
        ),
      ) as Promise<Awaited<ReturnType<typeof walletCtx.wallet.waitForSyncedState>>>,
    ]);
  } catch {
    state = await Rx.firstValueFrom(walletCtx.wallet.state()) as any;
    console.log('\n  ⚠ Strict sync timed out; continuing with latest wallet state.');
  }
  ...
  // Do not require facade isSynced — on public networks it may never flip true
  // even when unshielded balances are already visible.
  let dustState = await Rx.firstValueFrom(walletCtx.wallet.state());

  // Fresh faucet UTXOs need retroactive virtual DUST to accrue before
  // registerNightUtxosForDustGeneration can pay its own fee.
  if (network !== 'undeployed') {
    const waitMs = Number(process.env.MIDNIGHT_DUST_ACCRUAL_MS) || 120_000;
    console.log(`  Waiting ${Math.round(waitMs / 1000)}s for retroactive DUST accrual before registration...`);
    await new Promise((r) => setTimeout(r, waitMs));
    dustState = await Rx.firstValueFrom(walletCtx.wallet.state());
  }
  ...
  if (unregisteredUtxos.length > 0) {
    ...
    try {
      ...
      console.log(`  Registration submitted: ${txId ?? '(no id)'}`);
    } catch (err: any) {
      console.error(`  Registration failed: ${err?.message || err}`);
      console.error('  Will still wait for any existing DUST before aborting.');
    }
  } else {
    console.log('  All NIGHT UTXOs already registered for DUST (or none available).');
  }

  {
    const latest = await Rx.firstValueFrom(walletCtx.wallet.state());
    if (latest.dust.balance(new Date()) === 0n) {
      ...
      try {
        ...
        if (network !== 'undeployed') {
          ...
          console.log('    • Registration fee needed more retroactive DUST — re-run after waiting longer');
        }
        console.log('');
        await walletCtx.wallet.stop();
        process.exit(1);
  ...
  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    try {
      ...
      // initialPrivateState (raw claim for the privateClaim witness).
      // args is empty — VeilAttest has a no-arg constructor.
      deployed = await deployContract(providers, {
        compiledContract: compiledContract as any,
        args: [],
        privateStateId: PRIVATE_STATE_ID,
        initialPrivateState: { claim: INITIAL_CLAIM },
      });
      break;
    } catch (err: any) {
